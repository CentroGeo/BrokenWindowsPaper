...
DATA_FINAL_USE = MANZA_CDMX.merge(INTERSECCION,
                                  left_on = 'CVEGEO',
                                  right_on = 'CVEGEO',
                                  how = 'inner').rename({ "NOMUT": "NOM_COL", "ID": "ID_COL"}, 
                                                        axis = 1)
...
print(DESVE_ALCA.shape)
DESVE_ALCA.head(5)


```

# Variables de control uso de suelos

Para este caso se tienen las siguientes variables contabilizadas

```{python}
#| eval: true
#| echo: false

table = [["Número de estaciones de metro","Número de estaciones de metrobus"],
         ["Tiendas","Bares"],
         ["Restaurantes","Densidad de población(Km2)"]]
Markdown(tabulate(
  table, 
  headers=["Variables de control", ""]))
```

## Lineas de Metro

Procedemos a leer el archivo que contiene la información de las estaciones de metro y repoyectamos el sistema de coordendas a 6362; la reproyección la hacemos porque el archivo inicial viene proyectado en EPSG 32614 (UTM ZONA 14)
...
Ahora recordando la parte del join espacial procederemos a hacer uno con las estaciones del metro y las colonias, para porteriormente unir la informacion por clave de colonia con la base donde se encuentra toda la información y obtener asi el conteo por colonias y alcaldias.
...
Procedemos al conteo de las unidades en cada una de las colonias

```{python}
#| eval: true
METRO_COL = pd.DataFrame(METRO_CDMX.groupby(['CVE_COL']).agg({'CVE_EST':'count'}).reset_index())

METRO_COL.CVE_EST.value_counts()
```

Ahora con esta información podemos unir los resultados a la base donde se tiene la clave municipal y unir los resultados a nuestras tablas de desventajas e indices 
